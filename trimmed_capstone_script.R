#I have put all chunks of code (until the end) into functions with no arguments or return
...
#4. Code to create data frames to store time at which 50% of the fall traffic through each site
...
#   2.5 greater than that of all raptors combined
#7. Make maps from these data frames
#8. Extract date and magnitude of fall peak at each Hawk watch sites
# Note: due to formatting differences between the Hawk Watch data, most of this extraction was done
#   using code, but some was manual
...
addPopups(-68.25, 45.25, popup = paste("Restart animation"))
...
#as elements of a list, the indexing got too confusing and I couldn't get lapply to work
  ...
  if(manualpeaks2010[i,5]==0) {
    ...
    m2 <- m %>%
      ...
      addPopups(-68.25, 45.25, popup = paste("Year = 2010, Day = ",manualpeaks2010[i,3],
                                             ", Count = ", manualpeaks2010[i,4])) %>% # popup
...
map2011 <- function(i) {
  if(manualpeaks2011[i,5]==0) {
    m2 <- m %>%
      setView(-68.5, 44.4, 7) %>% # map location
      addMarkers(-68.25, 44.25) %>% # add a marker
      addPopups(-68.25, 45.25, popup = paste("ebird: Year = 2011, Day = ", manualpeaks2011[i,3], ", Count = ", manualpeaks2011[i,4])) %>% # popup
    ...
    m2 <- m %>%
      setView(-68.5, 44.4, 7) %>% # map location
      addMarkers(-68.25, 44.25) %>% # add a marker
      addPopups(-68.25, 45.25, popup = paste("Year = 2011, Day = ",manualpeaks2011[i,3], ", Count = ", manualpeaks2011[i,4])) %>% # popup
...
map2012 <- function(i) {
  if(manualpeaks2012[i,5]==0) {
    m2 <- m %>%
      setView(-68.5, 44.4, 7) %>% # map location
      addMarkers(-68.25, 44.25) %>% # add a marker
      addPopups(-68.25, 45.25, popup = paste("ebird: Year = 2012, Day = ",manualpeaks2012[i,3], ", Count = ", manualpeaks2012[i,4])) %>% # popup
    ...
    m2 <- m %>%
      setView(-68.5, 44.4, 7) %>% # map location
      addMarkers(-68.25, 44.25) %>% # add a marker
      addPopups(-68.25, 45.25, popup = paste("Year = 2012, Day = ",manualpeaks2012[i,3], ", Count = ", manualpeaks2012[i,4])) %>% # popup
...
map2013 <- function(i) {
  if(manualpeaks2013[i,5]==0) {
    m2 <- m %>%
      setView(-68.5, 44.4, 7) %>% # map location
      addMarkers(-68.25, 44.25) %>% # add a marker
      addPopups(-68.25, 45.25, popup = paste("Year = 2013, Day = ",manualpeaks2013[i,3], ", Count = ", manualpeaks2013[i,4])) %>% # popup
...
for(k in 2010:2013) {
  start_const <- 2007
  #start_const is chosen so that the first two columns in track are latitude and longitude, then the first year is the third column,
  #second year fourth column, etc. k is year, so have to take k-2007 to have first year go into third column
  for(i in 1:dim(x)[1]) {
    track[i,1] <- x[i,1]
    track[i,2] <- x[i,2]
    counts <-  
...
#I checked and these are correct
#now I need to write a loop that picks out, for each (non-zero) location
  ...
  make_map2010 <- function(i) {
    ...
    m2 <- m %>%
      ...
      addPopups(-68.25, 45.25, popup = paste("ebird:Year = ",ebird_years[[1]]$year[i], ", Day = ",
                                             ...
                                             ", Count = ", ebird_years[[1]]$halfcount[i])) %>% # popup
  ...
  make_map2011 <- function(i) {
    ...
    m2 <- m %>%
      ...
      addPopups(-68.25, 45.25, popup = paste("ebird: Year = ",ebird_years[[2]]$year[i], ", Day = ",
                                             ...
                                             ", Count = ", ebird_years[[2]]$halfcount[i])) %>% # popup
  ...
  make_map2012 <- function(i) {
    ...
    m2 <- m %>%
      ...
      addPopups(-68.25, 45.25, popup = paste("ebird: Year = ",ebird_years[[3]]$year[i], ", Day = ",
                                             ...
                                             ", Count = ", ebird_years[[3]]$halfcount[i])) %>% # popup
  ...
  make_map2013 <- function(i) {
    ...
    m2 <- m %>%
      ...
      addPopups(-68.25, 45.25, popup = paste("ebird: Year = ",ebird_years[[4]]$year[i], ", Day = ",
                                             ...
                                             ", Count = ", ebird_years[[4]]$halfcount[i])) %>% # popup
  ...
  make_map2010 <- function(i) {
    m2 <- m %>%
      ...
      addPopups(-68.25, 45.25, popup = paste("ebird: Year = 2010, Day = ",
                                             ...
                                             ", Count = ", setop_count_ten$`2010`[i])) %>% # popup
  ...
  make_map2011 <- function(i) {
    m2 <- m %>%
      ...
      addPopups(-68.25, 45.25, popup = paste("ebird: Year = 2010, Day = ",
                                             ...
                                             ", Count = ", setop_count_eleven$`2011`[i])) %>% # popup
...
setop2012 <- function() {
  make_map2012 <- function(i) {
    m2 <- m %>%
      setView(-68.5, 44.4, 6) %>% # map location
      addMarkers(-68.25, 44.25) %>% # add a marker
      addPopups(-68.25, 45.25, popup = paste("ebird: Year = 2012, Day = ",
                                             setop_day_twelve$`2012`[i],
                                             ", Count = ", setop_count_twelve$`2012`[i])) %>% # popup
...
make_map2013 <- function(i) {
  m2 <- m %>%
    setView(-68.5, 44.4, 7) %>% # map location
    addMarkers(-68.25, 44.25) %>% # add a marker
    addPopups(-68.25, 45.25, popup = paste("ebird: Year = 2013, Day = ",
                                           setop_day_thirteen$`2013`[i],
                                           ", Count = ", setop_count_thirteen$`2013`[i])) %>% # popup
...
peak_total <- data.frame()
  ...
  for(k in 2002:2013) {
    ...
    for(i in 2:length(sites)) { #excluding Acadia because Date formatting
      ...
      peak_total[i,k-start_const] <- max(counts$birdcount) #i.e. column 3
      for(j in 1:length(counts$birdcount)) {
        if(counts$birdcount[j] == max(counts$birdcount)) {
...
hawktwo <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2002, ", Day is ",
                                           peak_day_two$two[i],
                                           ", Count is ", peak_total_two$two[i])) %>% # popup
...
hawkthree <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2003, ", Day is ",
                                           ...
                                           ", Count is ", peak_total_three$three[i])) %>% # popup
...
hawkfour <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2004, ", Day is ",
                                           ...
                                           ", Count is ", peak_total_four$four[i])) %>% # popup
...
hawkfive <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2005, ", Day is ",
                                           ...
                                           ", Count is ", peak_total_five$five[i])) %>% # popup
...
hawksix <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2006, ", Day is ",
                                           peak_day_six$six[i],
                                           ", Count is ", peak_total_six$six[i])) %>% # popup
...
hawkseven <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2007, ", Day is ",
                                           ...
                                           ", Count is ", peak_total_seven$seven[i])) %>% # popup
...
hawkeight <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2008, ", Day is ",
                                           ...
                                           ", Count is ", peak_total_eight$eight[i])) %>% # popup
...
hawknine <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2009, ", Day is ",
                                           ...
                                           ", Count is ", peak_total_nine$nine[i])) %>% # popup
...
hawkten <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2010, ", Day is ",
                                           peak_day_ten$ten[i],
                                           ", Count is ", peak_total_ten$ten[i])) %>% # popup
...
hawkeleven <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2011, ", Day is ",
                                           ...
                                           ", Count is ", peak_total_eleven$eleven[i])) %>% # popup
...
hawktwelve <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2012, ", Day is ",
                                           ...
                                           ", Count is ", peak_total_twelve$twelve[i])) %>% # popup
...
hawkthirteen <- function(i) {
  ...
  m2 <- m %>%
    ...
    addPopups(-68.25, 45.25, popup = paste("Year is ",2013, ", Day is ",
                                           ...
                                           ", Count is ", peak_total_thirteen$thirteen[i])) %>% # popup
...
plot(wag_years, as.vector(peak_day_wag), main = "Waggoner's Gap. R square = 0.07256",
...
for(k in 1960:1999) {
  ...
  if(dim(counts)[1]>0) {
    ...
    #set index at k-1959 so that it starts at k==1
    doy <- strftime(temp_date, format = "%j")
...
plot(mpeter_years, peak_day_mpeter, main = "Mount Peter. Multiple R squared = 0.9371",
     xlab = "Year", ylab = "Date of peak count")
#Got info on how to do trendline in base package from:
#http://stackoverflow.com/questions/15102254/how-do-i-add-different-trend-lines-in-r
...
pilgrim_plot <- function() {
  ...
  plot(pilgrim_years, as.vector(peak_day_pilgrim),
       main = "Pilgrim Heights. Multiple R squared = 0.1554",
       xlab = "Year", ylab = "Date of peak count")
...
alleg_plot <- function() {
  ...
  plot(alleg_years, as.vector(peak_day_alleg),
       main = "Allegheny Front. Multiple R squared = 0.5434",
       xlab = "Year", ylab = "Date of peak count")
...
franklin_plot <- function() {
  ...
  plot(franklin_years, as.vector(peak_day_franklin),
       main = "Franklin Mountain. Multiple R squared = 0.01959",
       xlab = "Year", ylab = "Date of peak count")
...
#Manually recorded Acadia peak days 2002-2013
...
im.convert(c("~/a1.png", "~/a2.png", "~/a3.png", "~/a4.png", "~/a5.png", "~/a6.png", "~/a7.png", 
             "~/a8.png", "~/a9.png", "~/a10.png", "~/a11.png", "~/a12.png", "~/a13.png", "restart.png",
             "restart.png", "restart.png", "restart.png"),
           output = "ebird2010pause.gif" , convert = c("convert", "gm convert"), 
           cmd.fun = if (.Platform$OS.type == "windows") shell else system, extra.opts = "", 
           clean = FALSE)

im.convert(c("~/b1.png", "~/b2.png", "~/b3.png", "~/b4.png", "~/b5.png", "~/b6.png", "~/b7.png", 
             "~/b8.png", "~/b9.png", "restart.png",
             "restart.png", "restart.png", "restart.png"),
           output = "ebird2011pause.gif" , convert = c("convert", "gm convert"), 
           cmd.fun = if (.Platform$OS.type == "windows") shell else system, extra.opts = "", 
           clean = FALSE)


im.convert(c("~/c1.png", "~/c2.png", "~/c3.png", "~/c4.png", "~/c5.png", "~/c6.png", "~/c7.png", 
             "~/c8.png", "~/c9.png", "~/c10.png", "~/c11.png", "~/c12.png", "~/c13.png", "~/c14.png",
             "~/c15.png", "~/c16.png", "~/c17.png", "~/c18.png", "~/c19.png", "~/c20.png", "restart.png",
             "restart.png", "restart.png", "restart.png"),
           output = "ebird2012pause.gif" , convert = c("convert", "gm convert"), 
           cmd.fun = if (.Platform$OS.type == "windows") shell else system, extra.opts = "", 
           clean = FALSE)


im.convert(c("~/d1.png", "~/d2.png", "~/d3.png", "~/d4.png", "~/d5.png", "~/d6.png", "~/d7.png", 
             "~/d8.png", "~/d9.png", "~/d10.png", "~/d11.png", "~/d12.png", "~/d13.png", "restart.png",
             "restart.png", "restart.png", "restart.png"),
           output = "ebird2013pause.gif" , convert = c("convert", "gm convert"), 
           cmd.fun = if (.Platform$OS.type == "windows") shell else system, extra.opts = "", 
           clean = FALSE)
...
m2 <- m %>%
  setView(-68.5, 44.4, 6) %>% # map location
  addMarkers(-68.25, 44.25) %>% # add a marker
  addPopups(-68.25, 45.25, popup = "Restart animation")
m2

#the "restart.png" file has been modified here

im.convert(c("~/e1.png", "~/e2.png", "~/e3.png", "~/e4.png", "restart.png",
             "restart.png", "restart.png"),
           output = "ebird2010halfpause.gif" , convert = c("convert", "gm convert"), 
           cmd.fun = if (.Platform$OS.type == "windows") shell else system, extra.opts = "", 
           clean = FALSE)


im.convert(c("~/f1.png", "~/f2.png", "~/f3.png", "~/f4.png", "restart.png",
             "restart.png", "restart.png"),
           output = "ebird2011halfpause.gif" , convert = c("convert", "gm convert"), 
           cmd.fun = if (.Platform$OS.type == "windows") shell else system, extra.opts = "", 
           clean = FALSE)

im.convert(c("~/g1.png", "~/g2.png", "~/g3.png", "~/g4.png", "~/g5.png", "~/g5.png", "~/g6.png",
             "~/g7.png", "~/g8.png", "~/g9.png", "~/g10.png", "~/g11.png", "~/g12.png", "restart.png",
             "restart.png", "restart.png"),
           output = "ebird2012halfpause.gif" , convert = c("convert", "gm convert"), 
           cmd.fun = if (.Platform$OS.type == "windows") shell else system, extra.opts = "", 
           clean = FALSE)


im.convert(c("~/h1.png", "~/h2.png", "~/h3.png", "~/h4.png", "~/h5.png", "~/h5.png", "~/h6.png",
             "~/h7.png", "~/h8.png", "~/h9.png", "~/h10.png", "~/h11.png", "restart.png",
             "restart.png", "restart.png"),
           output = "ebird2013halfpause.gif" , convert = c("convert", "gm convert"), 
           cmd.fun = if (.Platform$OS.type == "windows") shell else system, extra.opts = "", 
           clean = FALSE)
...
counts <-  
  ...
  group_by(Date) %>%
  summarise(birdcount = sum(TOTAL))
plot(counts$Date,counts$birdcount)